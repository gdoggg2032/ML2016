...
class mf(object):
	...
	def add_model(self):
		with tf.device('/cpu:0'):
			...
			embedding_v = tf.concat(0, [tf.constant([[0.] * self.args.dim]*self.args.num_features, dtype=tf.float32), 
									 tf.Variable(tf.random_uniform(
									 	[self.args.max_features*self.args.num_features, self.args.dim], -.1, .1))
									])
		...
		with tf.device('/gpu:0'):
			...
			for i in range(self.args.num_features):
				for j in range(i+1, self.args.num_features):
					xx = tf.expand_dims(x[:,:,i]*self.args.num_features+j, 2)
					xxx = tf.expand_dims(x[:,:,j]*self.args.num_features+i, 2)
					xx_l.append(xx)
					xxx_l.append(xxx)

			xx_c = tf.concat(2, xx_l)
			xxx_c = tf.concat(2, xxx_l)

			embed_v_xx = tf.nn.embedding_lookup(embedding_v, xx_c)
			embed_v_xxx = tf.nn.embedding_lookup(embedding_v, xxx_c)

			# m = tf.reduce_sum(embed_v_xx * embed_v_xxx, [3])

			# mm = tf.reduce_sum(m, [2])
			mm = tf.reduce_sum(embed_v_xx * embed_v_xxx, [3])
