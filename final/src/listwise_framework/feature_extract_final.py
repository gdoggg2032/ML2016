...
def extract_dump(args):
	...
	print >> sys.stderr,day_train
	print >> sys.stderr,leak_count
	if args.mode == 0:
		dump(args, args.click_va, args.va_fm, feature_dict, header_dict, train=False)
		print >> sys.stderr,day_valid
		print >> sys.stderr,leak_count
	else:
	...
	print >> sys.stderr,day_train, day_valid
	with open("./feature_num", "w") as f:
		f.write(str(len(hash_dict)))
	# print >> sys.stdout, len(hash_dict)
...
def data(args, data_path, feature_dict, header_dict, train=True):
	...
	for t, row in enumerate(DictReader(open(data_path, "r"))):
		...
		# # source id same or not
		# if display_doc_id in doc_meta_dict \
		# and ad_doc_id in doc_meta_dict \
		# and doc_meta_dict[display_doc_id]['source_id'] == doc_meta_dict[ad_doc_id]['source_id']:
		# 	x.append(d_hash("source_id_are_the_same", train))
		# 	leak_count[10] += 1
		# else:
		# 	x.append(d_hash("source_id_are_not_the_same", train))
		# 	leak_count[11] += 1

		# # publisher_id same or not
		# if display_doc_id in doc_meta_dict \
		# and ad_doc_id in doc_meta_dict \
		# and doc_meta_dict[display_doc_id]['publisher_id'] == doc_meta_dict[ad_doc_id]['publisher_id']:
		# 	x.append(d_hash("publisher_id_are_the_same", train))
		# 	leak_count[12] += 1
		# else:
		# 	x.append(d_hash("publisher_id_are_not_the_same", train))
		# 	leak_count[13] += 1

		# ad publish_time is later than display doc publish_time
		if display_doc_id in doc_meta_dict \
		and ad_doc_id in doc_meta_dict \
		and (doc_meta_dict[ad_doc_id]['publish_time'] - doc_meta_dict[display_doc_id]['publish_time']).days >= 0:
			x.append(d_hash("ad_publish_time_later_than_display_doc", train))
			leak_count[14] += 1
		else:
			x.append(d_hash("ad_publish_time_not_later_than_display_doc", train))
			leak_count[15] += 1

		# ad publish_time is earlier than event time
		if ad_doc_id in doc_meta_dict \
		and (datetime.datetime(2016,6,event_day,event_hour,0) - doc_meta_dict[ad_doc_id]['publish_time']).days >= 0:
			x.append(d_hash("ad_publish_time_earlier_than_event_time", train))
			leak_count[16] += 1
		else:
			x.append(d_hash("ad_publish_time_not_earlier_than_event_time", train))
			leak_count[17] += 1
		...
		x = x[:args.num_features]

		# check if x not len == 12
		if len(x) != args.num_features:
			print >> sys.stderr,"len x error ", len(x)
