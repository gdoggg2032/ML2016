...
class mf(object):
	...
	def add_model(self):
		...
		with tf.device('/gpu:0'):
			...
			w_mm = tf.Variable(tf.random_uniform([w_mm_dim, 500], -.1, .1))
			b_mm = tf.Variable(tf.random_uniform([500], -.1, .1))
			a_mm = tf.matmul(mm_reshape, w_mm) + b_mm


			# w_mm = tf.Variable(tf.random_uniform([100, 100], -.1, .1))
			# b_mm = tf.Variable(tf.random_uniform([100], -.1, .1))
			# a_mm = tf.matmul(a_mm, w_mm) + b_mm

			w_mm = tf.Variable(tf.random_uniform([500, 1], -.1, .1))
