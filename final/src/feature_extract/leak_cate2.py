...
for c, row in enumerate(csv.DictReader(open("../input_data/documents_categories.csv"))):
    ...
    if confidence_level > 0.3:
        doc_cate_dict[doc_id].add(int(row['category_id']))
...
for c,row in enumerate(csv.DictReader(open('../input_data/promoted_content.csv'))):
    if row['document_id'] != '':
        doc_id = int(row['document_id'])
        if doc_id in doc_cate_dict:
            for cate in doc_cate_dict[doc_id]:
                leak[cate] = 1
        else:
            c += 1
print(len(leak))
